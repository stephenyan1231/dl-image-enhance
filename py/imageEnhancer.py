...
#         op.add_option("segment-dir", 'img_seg_dir', \
#                       StringOptionParser, "folder where segmentation files reside", default="")
...
#         op.add_option("in-img-context-dir", 'in_img_context_dir', StringOptionParser,
#                       "overwrite image context feature folder in batches.meta", default='')         
#         op.add_option("in-img-precomputed-context-ftr-dir", "in_img_precomputed_context_ftr_dir", \
#                       StringOptionParser, "folder to save precomputed pixel local context feature", default="")      
#         op.add_option("in-img-precomputed-context-mean-color-ftr-dir", "in_img_precomputed_context_mean_color_ftr_dir", \
#                       StringOptionParser, "folder to save precomputed pixel context mean color feature", default="")         
...
#             print 'label_layer_idx', self.label_layer_idx                            
        self.tr_batches_meta = self.train_data_provider.get_batches_meta()
        if self.regress_L_channel_only == 1:
            print 'regress L channel only'
        else:
            print 'regress L,a,b channels'
        if self.use_local_context_ftr:
            print 'ImageEnhancer init_model_state. Use local context feature'
        else:
            print 'ImageEnhancer init_model_state. Do not use local context feature'            
        ...
        if self.test_image_id_file:
            self.in_img_id = readImgNameFlie(self.test_image_id_file, self.tr_batches_meta['in_imgs'])
            
#             print 'test image id file: %s' % self.test_image_id_file
#             ts_img_id_file = open(self.test_image_id_file, 'r')
#             self.in_img_id = []
#             for line in ts_img_id_file:
#                 self.in_img_id += [int(line) - 1]
#             ts_img_id_file.close()
        else:
            self.in_img_id = self.tr_batches_meta['ts_img_id']
        
        self.in_imgs = [self.tr_batches_meta['in_imgs'][id] for id in self.in_img_id]
        self.enh_imgs = [self.tr_batches_meta['enh_imgs'][id] for id in self.in_img_id]
        if self.use_local_context_ftr:
            self.semIntegralMapFiles = [self.tr_batches_meta['semIntegralMapFiles'][id] for id in self.in_img_id]                
            self.semMapFiles = [self.tr_batches_meta['semMapFiles'][id] for id in self.in_img_id]                
            if self.use_local_context_color_ftr:
                self.colorIntegralFiles = [self.tr_batches_meta['colorIntegralFiles'][id] for id in self.in_img_id]                
        ...
        if not hasattr(self, 'use_position_ftr'):
...
#             local_context_color_paras = self.tr_batches_meta['local_context_color_paras']
        ...
        print 'in_img_dir,enh_img_dir,in_img_context_dir', \
        self.in_img_dir, self.enh_img_dir, self.sem_integral_map_dir
        ...
        for i in range(num_in_imgs):
            st = time.time()
            print 'processing %d th out of %d images: %s' % (i, num_in_imgs, self.in_imgs[i])
            if self.fredo_image_processing == 1:
                in_img = read_tiff_16bit_img_into_LAB(os.path.join(self.in_img_dir, self.in_imgs[i]), 1.5, False)
                enh_img = read_tiff_16bit_img_into_LAB(os.path.join(self.enh_img_dir, self.enh_imgs[i]), 0, False)                
            else:
                in_img = read_tiff_16bit_img_into_LAB(os.path.join(self.in_img_dir, self.in_imgs[i]))
                enh_img = read_tiff_16bit_img_into_LAB(os.path.join(self.enh_img_dir, self.enh_imgs[i]))
            if self.super_pixel_enhance:
                in_img_seg = load_segment(os.path.join(self.img_seg_dir, self.in_imgs[i][:-4] + '.seg'), in_img.shape)
...
#                 posx, posy = get_samples_pos(h, w, patch_hs, 1)
    #             print 'posx shape', posx.shape
            ...
            print 'label_dim %d. %d batches for %d th image' % (label_dim, num_batch, i)
            side = 2 * patch_hs + 1
            ...
            if self.use_local_context_ftr:
                if 0:
#                     print 'in_img_precomputed_context_ftr_dir',self.in_img_precomputed_context_ftr_dir
                    precomputed_context_ftr_matfile = os.path.join\
                    (self.in_img_precomputed_context_ftr_dir, self.in_imgs[i][:-4] + '_context_ftr.mat')
                    print 'load mat file %s' % precomputed_context_ftr_matfile
                    precomputed_context_ftr = scipy.io.loadmat(precomputed_context_ftr_matfile)
                    precomputed_context_ftr = precomputed_context_ftr['contextSemFtr']
                else:
                ...
                if self.use_local_context_color_ftr:
                    colorIntegralMap_ = scipy.io.loadmat(os.path.join(self.img_color_integral_dir, self.colorIntegralFiles[i]))
                    colorIntegralMap = colorIntegralMap_['colorIntegralMaps']
                    pixNumIntegralMap = colorIntegralMap_['pixNumIntegralMaps']
                    
#             if self.use_local_context_color_ftr:
#                 #print self.ori_img_precomputed_context_mean_color_ftr_dir
#                 precomputed_context_mean_color_matfile = os.path.join\
#                 (self.in_img_precomputed_context_mean_color_ftr_dir, self.in_imgs[i][:-4] + '_context_mean_color_ftr.mat')
#                 print 'load mat file %s' % precomputed_context_mean_color_matfile
#                 precomputed_context_mean_color_ftr = scipy.io.loadmat(precomputed_context_mean_color_matfile)
#                 ''' precomputed_context_mean_color_ftr shape: (h,w,ftr_dim=75) '''
#                 precomputed_context_mean_color_ftr = precomputed_context_mean_color_ftr['context_mean_color_ftr']                
            ...
            if not self.super_pixel_enhance:
                ...
                pix_ftr = np.zeros((pix_ftr_dim, self.NUM_ROWS_PER_BATCH * w), dtype=np.single)
                pix_ftr2 = np.zeros((self.NUM_ROWS_PER_BATCH, w, CENTRAL_PX_FEATURE_DIM), dtype=np.single)
...
#                         print 'start_row,end_row,h,batch',start_row,end_row,h,batch
                        pix_ftr = pix_ftr[:, :next_batch_size]
                        pix_ftr2 = pix_ftr2[:end_row - start_row, :, :]
                        
                        get_pixel_feature\
                        (in_img[start_row:end_row, :, :], \
                         self.tr_batches_meta['pixel_feature_neighbor_half_size'], pix_ftr2)
                        pix_ftr2 = pix_ftr2.reshape((next_batch_size, pix_ftr2.shape[2]))
                        
                        pix_ftr[:pix_ftr2.shape[1], :] = pix_ftr2.transpose()
                        pix_ftr2 = pix_ftr2.reshape((end_row - start_row, w, CENTRAL_PX_FEATURE_DIM))
...
#                             print 'contextSemFtr sum',np.sum(contextSemFtr[0,:].reshape((25,20)),axis=1)
                        ...
                        if self.use_local_context_color_ftr:
                            ...
                            context_mean_color_ftr = precomputed_context_mean_color_ftr[start_row:end_row, :, :]
                            context_mean_color_ftr = context_mean_color_ftr.reshape((end_row - start_row) * w, 3 * local_context_paras['pool_region_num'])                            
                        ...
                        if self.use_position_ftr:
                            pix_x, pix_y = np.meshgrid(range(w), range(start_row, end_row))
                            pix_x, pix_y = pix_x.flatten(), pix_y.flatten()
                            normed_pix_x, normed_pix_y = \
                            np.single(pix_x) / np.single(w), np.single(pix_y) / np.single(h)
                            pix_pos = np.vstack((normed_pix_x, normed_pix_y))                           
                        
                        if self.use_local_context_ftr:
                            context_ftr_t = contextSemFtr.transpose()
                            if self.use_position_ftr:
                                aux_data = [[self.in_img_id[i]] * next_batch_size, gt_color, context_ftr_t, pix_pos]                                
                            else:
                                if self.use_local_context_color_ftr:
                                    local_context_color_ftr_t = local_context_color_ftr.transpose()
                                    context_mean_color_ftr_t = context_mean_color_ftr.transpose()
                                    aux_data = [[self.in_img_id[i]] * next_batch_size, gt_color, context_ftr_t, \
                                                local_context_color_ftr_t, context_mean_color_ftr_t]
                                else:
                                    aux_data = [[self.in_img_id[i]] * next_batch_size, gt_color, context_ftr_t]
                        else:
                            if self.use_position_ftr:
                                aux_data = [[self.in_img_id[i]] * next_batch_size, gt_color, pix_pos]
                            else:
                                aux_data = [[self.in_img_id[i]] * next_batch_size, gt_color]
                ...
                print 'predicting enhancement elapsed time:%f elapsed_time_data_prepare %f, elapsed_local_context: %f elapsed_dp:%f'\
                % (elapsed_time, elapsed_time_data_prepare, elapsed_local_context, elapsed_dp)
                print 'elapsed_dp1:%f elapsed_dp2:%f' % (elapsed_dp1, elapsed_dp2)

                regress_ch = label_dim / basis_dim
                predLb = predLb.reshape((h, w, regress_ch, basis_dim))
                if self.transform_vis:
                    l_pred_labels = np.zeros((h, w,regress_ch))
                    for j in range(basis_dim):
                        out_vis_path = os.path.join(self.out_transform_vis_dir, self.in_imgs[i][:-4] + '_vis_' + '%d' % j + '.png')
                        l_pred_labels[:, :, :] = predLb[:, :, :, j]
                        for k in range(regress_ch):
                            l_min,l_max=np.min(l_pred_labels[:,:,k]),np.max(l_pred_labels[:,:,k])
                            l_pred_labels[:,:,k] = (l_pred_labels[:,:,k] - l_min) / (l_max - l_min)
...
#                         print 'start_pos,end_pos,next_batch_size,batch', start_pos, end_pos, next_batch_size, batch
                        pix_ftr = pix_ftr[:next_batch_size, :]
                        get_pixel_feature_v2\
                        (in_img, l_posx, l_posy, self.tr_batches_meta['pixel_feature_neighbor_half_size'], \
                         pix_ftr[:, :CENTRAL_PX_FEATURE_DIM])
#                             print 'pix_ftr ',pix_ftr[0,:]
#                             print 'pix_ftr ',pix_ftr[next_batch_size-1,:]
                        patches = pix_ftr.transpose()                            
                            
                        gt_color = enh_img[l_posy, l_posx, :].reshape((next_batch_size, 3))
                        gt_color = gt_color.swapaxes(0, 1)
                        
                        if self.use_local_context_color_ftr:
                            contextColorFtr, contextPixNumFtr = \
                            getPixContextColorFtr(l_posx, l_posy, semMap[l_posy, l_posx], colorIntegralMap, pixNumIntegralMap, self.tr_batches_meta['local_context_paras'])
                        ...
                        if self.use_local_context_ftr:
                            if 0:
                                contextSemFtr = precomputed_context_ftr[l_posy, l_posx, :]
                            else:
                                contextSemFtr = contextSemFtr[:next_batch_size, :]
                            ...
                            if self.use_local_context_color_ftr:
                                next_data_all = self.train_data_provider.prepare_batch_data\
                                (patches, [[self.in_img_id[i]] * next_batch_size, gt_color, \
                                           contextSemFtr.transpose(), contextColorFtr.transpose(), \
                                           contextPixNumFtr.transpose()])                          
                            else:
                                next_data_all = self.train_data_provider.prepare_batch_data\
                                (patches, [[self.in_img_id[i]] * next_batch_size, gt_color, contextSemFtr.transpose()])                            
                        else:
                            next_data_all = self.train_data_provider.prepare_batch_data\
                            (patches, [[self.in_img_id[i]] * next_batch_size, gt_color])
                        next_data, elapsed_dp_12 = next_data_all[0], next_data_all[1]
                    ...
                    if start_write == 1:
                        batch_output = self.finish_batch()
                        predLb[p:p + batch_size, :] = l_pred_labels
#                         print 'l_pred_labels shape',l_pred_labels.shape
#                         print l_pred_labels
                        p += batch_size
                        ...
                        if self.regress_L_channel_only:
                            ...
                            if self.super_pixel_enhance_uniform_color:
                                if self.super_pixel_enhance_uniform_color == 1:
                                    segW, segH = seg % w, seg / w
                                    ctrW, ctrH = np.median(segW), np.median(segH)
                                    dist = (segW - ctrW) ** 2 + (segH - ctrH) ** 2
                                    idx = np.argsort(dist)                                    
                                    out_img[seg, :] = l_pred_colors[:, idx[0]]
                                else:
...
#                         l_seg_cx, l_seg_cy = np.mean(l_seg_x), np.mean(l_seg_y)
                ...
                enh_img = enh_img.reshape((h, w, ch))
#             print 'L min max', np.min(out_img[:, :, 0]), np.max(out_img[:, :, 0])
#            out_img_path = os.path.join(self.out_img_dir, self.in_imgs[i])
#            saveLabIntoTiff16bitImg(out_img_path, out_img)
            out_img_path = os.path.join(self.out_img_dir, self.in_imgs[i][:-4] + ".png")            
            ...
            % (self.in_imgs[i], L_dist[i], L_RMSE[i], Lab_dist[i], Lab_RMSE[i])             
        ...
        imgToLabDist = {}
        for i in range(num_in_imgs):
            imgToLabDist[self.in_imgs[i][:-4]] = Lab_dist[i]
...
#         print 'div', div[1:2, :10, :]

#         print 'grad_mag',grad_mag[:10,:10,:]
    ...
    def do_Poisson_reconstruct(self):
        ...
        cur_dir = os.getcwd()
#         self.out_img_dir
#         self.poisson_reconstruct_data_dir
#         self.ori_img_edge_folder
#         self.in_img_precomputed_context_ftr_dir
#         self.in_img_dir
...
#         for i in range(19, 21):
            ...
            (self.poisson_reconstruct_data_dir, self.in_imgs[i][:-4])
            ...
            if self.fredo_image_processing == 1:
                ...
                (os.path.join(self.in_img_dir, self.in_imgs[i]), 1.5, False)
                gt_enh_img = read_tiff_16bit_img_into_LAB\
                (os.path.join(self.enh_img_dir, self.enh_imgs[i]), 0, False)                
            else:
                in_img = read_tiff_16bit_img_into_LAB\
                (os.path.join(self.in_img_dir, self.in_imgs[i]))
                gt_enh_img = read_tiff_16bit_img_into_LAB\
                (os.path.join(self.enh_img_dir, self.enh_imgs[i]))            
            assert in_img.shape == gt_enh_img.shape
            h, w, ch = in_img.shape[0], in_img.shape[1], in_img.shape[2]
            
            in_img_L = in_img[:, :, 0]
#             print 'in_img_L min,max',np.min(in_img_L),np.max(in_img_L)
            in_img_grady_L, in_img_gradx_L = np.gradient(in_img_L)
            in_img_gradmag_L = np.sqrt(in_img_grady_L ** 2 + in_img_gradx_L ** 2)
#             print 'in_img_gradmag_L min,max,mean',np.min(in_img_gradmag_L),np.max(in_img_gradmag_L), np.mean(in_img_gradmag_L)
...
#             print 'gt_enh_img_gradmag_L min,max,mean',np.min(gt_enh_img_gradmag_L), np.max(gt_enh_img_gradmag_L), np.mean(gt_enh_img_gradmag_L)            
            
            pred_enh_img_path = os.path.join(self.out_img_dir, self.in_imgs[i][:-4] + '.png')
            pred_enh_im_sRGB = scipy.misc.imread(pred_enh_img_path)
            pred_enh_im_Lab = color.rgb2lab(pred_enh_im_sRGB)
                        
#             print 'pred_enh_im_Lab L min,max a,min,max',np.min(pred_enh_im_Lab[:,:,0]),\
#             np.max(pred_enh_im_Lab[:, :, 0]), np.min(pred_enh_im_Lab[:, :, 1]), np.max(pred_enh_im_Lab[:, :, 1])
...
#             print 'pred_enh_im_Lab_normed L min,max a,min,max',np.min(pred_enh_im_Lab_normed[:,:,0]),\
#             np.max(pred_enh_im_Lab_normed[:, :, 0]), np.min(pred_enh_im_Lab_normed[:, :, 1]), np.max(pred_enh_im_Lab_normed[:, :, 1])
            
            img_edge_mat_file_path = \
            os.path.join(self.ori_img_edge_folder, self.in_imgs[i][:-4] + '_edge.mat')            
...
#                 print 'use precomptued context ftr'
                ...
                print 'compute context ftr on the fly'
                context_map = \
                scipy.io.loadmat(os.path.join(self.sem_integral_map_dir, self.semIntegralMapFiles[i]))
...
#             edge_pix_context_ftr = edge_pix_context_ftr.reshape((batch_size,local_context_paras['ftr_dim']))
#             print 'edge_pix_context_ftr',edge_pix_context_ftr[:2,:]
#             print 'edge_pix_context_ftr shape',edge_pix_context_ftr.shape
...
#             print 'gradmag_transform', gradmag_transform[:4, :]
            pred_edge_pix_gradmag_L = np.exp(gradmag_transform[:, 0] * np.log(edge_pix_L) + gradmag_transform[:, 1])\
            *edge_pix_gradmag_L
#             print 'pred_edge_pix_gradmag_L min,max,mean', np.min(pred_edge_pix_gradmag_L), \
            np.max(pred_edge_pix_gradmag_L), np.mean(pred_edge_pix_gradmag_L)
#             print 'pred_edge_pix_gradmag_L', pred_edge_pix_gradmag_L[:10]
#             print 'gt_enh_edge_pix_gradmag_L', gt_enh_edge_pix_gradmag_L[:10]
...
#             print 'in_img_normed',in_img_normed[:3,:6,0]
            if 1:
#                 in_img_normed_smooth = np.zeros_like(in_img_normed)
#                 in_img_normed_smooth[:, :, 0] = scipy.ndimage.filters.gaussian_filter(in_img_normed[:, :, 0], gaus_smooth_sigma)                
#                 in_img_normed_smooth[:, :, 1] = scipy.ndimage.filters.gaussian_filter(in_img_normed[:, :, 1], gaus_smooth_sigma)                
#                 in_img_normed_smooth[:, :, 2] = scipy.ndimage.filters.gaussian_filter(in_img_normed[:, :, 2], gaus_smooth_sigma)                
...
#                 print 'grad_xy shape', grad_xy.shape
                if 0:
                    grady_L, gradx_L = np.gradient(in_img_normed_smooth[:, :, 0])
                    grady_a, gradx_a = np.gradient(in_img_normed_smooth[:, :, 1])
                    grady_b, gradx_b = np.gradient(in_img_normed_smooth[:, :, 2])                
            else:
                grady_L, gradx_L = np.gradient(pred_enh_im_Lab_normed[:, :, 0])
                grady_a, gradx_a = np.gradient(pred_enh_im_Lab_normed[:, :, 1])
                grady_b, gradx_b = np.gradient(pred_enh_im_Lab_normed[:, :, 2])
#             print 'gradx_L',gradx_L[:2,:3]
#             print 'grady_L',grady_L[:2,:3]
...
#             print 'grad_mag_L min,max',np.min(grad_mag_L),np.max(grad_mag_L)
...
#             mpplot.figure()
#             mpplot.imshow(new_img_sRGB)
#             mpplot.show()
            
            scipy.misc.imsave\
            (os.path.join(new_img_save_dir, self.in_imgs[i][:-4] + '.png'), new_img_sRGB)
            print '--------------------------------------------------------'
